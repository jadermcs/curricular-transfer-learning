...
def get_db_values(value_set_path): # value_set.json, all the domain[slot] values in datasets
    ...
    with open('data/db/ontology.json', 'r') as f: # read ontology in lower, all the domain-slot values
    ...
    with open('data/db/bspn_word_collection.json', 'w') as f:
...
class DataPreprocessor(object):
    def __init__(self):
        ...
        data_path = 'data/db/annotated_user_da_with_span_full.json'
        archive = zipfile.ZipFile(data_path + '.zip', 'r')
        self.convlab_data = json.loads(archive.open(data_path.split('/')[-1], 'r').read().lower())
        self.delex_sg_valdict_path = 'data/db/delex_single_valdict.json'
        self.delex_mt_valdict_path = 'data/db/delex_multi_valdict.json'
        self.ambiguous_val_path = 'data/db/ambiguous_values.json'
        self.delex_refs_path = 'data/db/reference_no.json'
    ...
    def preprocess_main(self, save_path=None, is_test=False):
        ...
        self.vocab.save_vocab('data/db/vocab')
        with open('data/db/dialog_acts.json', 'w') as f:
            json.dump(ordered_sysact_dict, f, indent=2)
        with open('data/db/dialog_act_type.json', 'w') as f:
...
if __name__=='__main__':
    db_paths = {
            'attraction': 'data/db/attraction_db.json',
            'hospital': 'data/db/hospital_db.json',
            'hotel': 'data/db/hotel_db.json',
            'police': 'data/db/police_db.json',
            'restaurant': 'data/db/restaurant_db.json',
            'taxi': 'data/db/taxi_db.json',
            'train': 'data/db/train_db.json',
        }
    get_db_values('data/db/value_set.json') # 
